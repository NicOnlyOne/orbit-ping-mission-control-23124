        ...
        <div className="mb-12">
          ...
          {user ? <div className="space-y-6">
              ...
              {monitorsLoading ? <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                  ...
                  {monitors.map(monitor => <MissionCard key={monitor.id} name={monitor.name} url={monitor.url} status={monitor.status} uptime={`${monitor.uptime_percentage}%`} responseTime={monitor.response_time ? `${monitor.response_time}ms` : 'N/A'} monitoringInterval={monitor.monitoring_interval} notifyEmail={monitor.notify_email} onTest={() => testMonitor(monitor.id)} onDelete={() => deleteMonitor(monitor.id)} onIntervalChange={interval => updateMonitorInterval(monitor.id, interval)} onEmailUpdate={email => updateMonitorEmail(monitor.id, email)} lastChecked={monitor.last_checked} />)}
